...
tool called `crate2nix` (`nix-env -iA nixpkgs.crate2nix`), which can be used
...
Now, the file produced by the call to `crate2nix`, called `Cargo.nix`, looks like:
...
If we look at a more complicated dependencies, for instance by adding a single
line `libc="*"` to our `Cargo.toml`, we first need to run `cargo build` to
update the `Cargo.lock`. Then, `carnix` needs to be run again, and produces the
following nix file:
