...
let
  ...
  xtensa-src = fetchFromGitHub {
    # https://github.com/espressif/binutils-gdb/commit/2802c5b8bd5d94826364f42f33cebd2dede6588d
    # https://github.com/espressif/binutils-gdb/commits/esp-2022r1-binutils
    owner = "espressif";
    repo = "binutils-gdb";
    rev = "2802c5b8bd5d94826364f42f33cebd2dede6588d";
  ...
  src-target = {
    ...
    # NOTE: this is the generic xtensa source code. esp32s3 src is formed by
    # copied files from xtensa-overlays into the source tree
    esp8266-none = xtensa-src;
    esp32-none = xtensa-src;
    esp32s2-none = xtensa-src;
    esp32s3-none = xtensa-src;
  };
...
stdenv.mkDerivation {
  ...
  srcs = stdenv.__bootPackages.binutils-unwrapped.srcs or ([
    src-target.${targetPlatform.system}
    or src-target.normal
  ] ++ lib.optionals stdenv.targetPlatform.isEsp [
    # NOTE: this same overlay is used for esp32 and esp32s2
    # https://github.com/espressif/xtensa-overlays/releases/tag/esp-2022r1
    # https://github.com/espressif/xtensa-overlays/commit/dd1cf19f6eb327a9db51043439974a6de13f5c7f
    fetchFromGitHub {
      owner = "espressif";
      repo = "xtensa-overlays";
      rev = "dd1cf19f6eb327a9db51043439974a6de13f5c7f";
      sha256 = lib.fakeSha256;
      name = "xtensa-overlays";
    }
  ]);

  # NOTE: we rely on overlays being unpacked last
  #unpackCmd = ''
  #  mkdir -p ./all
  #  if [[ $curSrc == *binutils* ]]; then
  #    cp -r $curSrc ./all
  #  fi
  #'' + (lib.optionalString targetPlatform.isEsp32s3 ''
  #  if [[ $curSrc == *xtensa-overlays* ]]; then
  #    cp -f $curSrc/xtensa_esp32s3/binutils/* ./all
  #  fi
  #'') + (lib.optionalString targetPlatform.isEsp32s2 ''
  #  if [[ $curSrc == *xtensa-overlays* ]]; then
  #    cp -f $curSrc/xtensa_esp32s2/binutils/* ./all
  #  fi
  #'');
